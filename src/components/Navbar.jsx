...
export default function Navbar() {
  ...
  const cartCount = useMemo(() => {
    return (Array.isArray(items) ? items : []).reduce((n, i) => n + (Number(i.quantity) || 0), 0);
  }, [items]);
    ...
    >
      ...
      <div className="max-w-6xl mx-auto px-4 py-3 flex items-center gap-4">
        ...
        <div className="hidden md:flex items-center gap-2">
          <Link href="/dashboard" className={`${linkClass} ${pathname === '/dashboard' ? 'text-blue-600' : ''}`}>
            {t('nav.dashboard')}
          </Link>
          <Link href="/products" className={`${linkClass} ${pathname === '/products' ? 'text-blue-600' : ''}`}>
            {t('nav.products')}
          </Link>
          <Link href="/orders" className={`${linkClass} ${pathname === '/orders' ? 'text-blue-600' : ''}`}>
            {t('nav.orders')}
          </Link>
          <Link
            href="/orders/new"
            className={`${linkClass} ${pathname === '/orders/new' ? 'text-blue-600' : ''} relative inline-flex items-center`}
            aria-label="Cart"
          >
          ...
          {!isAuthenticated ? (
            <Link href="/login" className={`${linkClass} ${pathname === '/login' ? 'text-blue-600' : ''}`}>
              {t('nav.login')}
            </Link>
          ) : (
            <button
              onClick={logout}
              className="ml-2 px-3 py-1 rounded bg-gray-900 text-white hover:opacity-90 transition-opacity"
            >
              {t('logout')}
            </button>
          )}
          ...
          <button
            onClick={toggleLang}
            className="px-2 py-1 rounded border text-sm hover:bg-black/5 transition-colors"
      ...
      {mobileOpen && (
        <div className="md:hidden border-t bg-white/80 backdrop-blur">
          <div className="max-w-6xl mx-auto px-4 py-3 flex flex-col gap-2">
            <Link
              href="/dashboard"
              className={`${linkClass} ${pathname === '/dashboard' ? 'text-blue-600' : ''}`}
            ...
            <Link
              href="/products"
              className={`${linkClass} ${pathname === '/products' ? 'text-blue-600' : ''}`}
              onClick={() => setMobileOpen(false)}
            >
              {t('nav.products')}
            </Link>
            <Link
              href="/orders"
              className={`${linkClass} ${pathname === '/orders' ? 'text-blue-600' : ''}`}
              onClick={() => setMobileOpen(false)}
            >
              {t('nav.orders')}
            </Link>
            <Link
              href="/orders/new"
              className={`${linkClass} ${pathname === '/orders/new' ? 'text-blue-600' : ''} inline-flex items-center gap-2`}
              onClick={() => setMobileOpen(false)}
              aria-label="Cart"
            >
            ...
            {!isAuthenticated ? (
              <Link
                href="/login"
                className={`${linkClass} ${pathname === '/login' ? 'text-blue-600' : ''}`}
                onClick={() => setMobileOpen(false)}
              >
                {t('nav.login')}
              </Link>
              ...
              <button
                ...
                className="px-3 py-1 rounded bg-gray-900 text-white w-full text-left"
              >
                {t('logout')}
              </button>
            )}
            ...
            <button
              ...
              className="px-2 py-1 rounded border text-sm hover:bg-black/5 transition-colors w-max"
              aria-label="Toggle language"
            >
              {(i18n.language || 'en').toUpperCase()}
            </button>
